...
- 위 세 가지 케이스로 사용 가능
  ...
  | 인스턴스::인스턴스메서드 | 인스턴스 메서드의 인자가 된다                                                        | System.out::println == sqrt -> System.out.println(sqrtNum) |
  | 클래스::인스턴스메서드   | 첫 번째 인자는 인스턴스가 되고 그 다음 인자(들)는 인스턴스 메서드의 인자(들)가 된다. | Integer::compareTo == (a, b) -> a.compareTo(b)             |

- 결국 메서드 레퍼런스는 람다식으로 변형되고, 최종적으로는 함수형 인터페이스가 된다.
  <br>

- 정리
  - 변수는 지역 변수, 속성, 메서드의 인자, 메서드의 반환값으로 사용 가능
  - 즉, 람다도 지역 변수, 속성, 메서드의 인자, 메서드의 반환값으로 사용 가능
  - 기존의 자바에서의 로직은 메서드를 통해서만 구현 가능
  - 자바 8에서는 로직을 람다로 표현 가능, 람다가 메서드의 역할도 할 수 있다.
  - 더 이상 로직을 객체 메서드나 정적 메서드 안에서 구현하지 않아도 된다.
  - 람다를 사용하면 메서드를 사용하기 위해서 더 이상 클래스를 선언하지 않아도 됨

<br><br><br>

- 에필로그
  - 책 추천:
    - [Taeyo's ASP] - 김태영 -> 웹 개발자로 만들어 준 책
    - [MS-SQL] - 정원혁 -> DB에 대한 개념을 잡아줌
    - [토비의 스프링] - 이일민 -> 개발 지식들이 어떨게 스프링에 녹아 있는 지, 웹 개발의 초석을 마련해준 책
